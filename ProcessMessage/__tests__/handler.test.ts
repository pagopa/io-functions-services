...
describe("getprocessMessageHandler", () => {
  ...
  it.each`
    ...
    ${"message sender special service does not exists in user service preference and Activation is PENDING"}  | ${"SENDER_BLOCKED"}        | ${O.some(aRetrievedProfileWithManualPreferences)}                                                             | ${O.none}                                                        | ${O.some({ ...aDisabledActivation, status: ActivationStatusEnum.PENDING })} | ${aCreatedMessageEvent} | ${aSpecialMessageData}
    ...
    ${"message sender special service exists in user service preference and Activation is PENDING"}           | ${"SENDER_BLOCKED"}        | ${O.some(aRetrievedProfileWithManualPreferences)}                                                             | ${O.some(anEnabledServicePreference)}                            | ${O.some({ ...aDisabledActivation, status: ActivationStatusEnum.PENDING })} | ${aCreatedMessageEvent} | ${aSpecialMessageData}
  `(
