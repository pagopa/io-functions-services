...
export function* handleMessage(
  ...
  const maybeAllowedEmailNotification = isEmailBlockedForService
    ...
    : getEmailAddressFromProfile(profile)
        ...
        .orElse(() => {
          winston.debug(
            `handleMessage|User profile has no email address set and no default address was provided|${newMessageWithoutContent.fiscalCode}`
          );
          return none;
        });
  ...
  if (noChannelsConfigured) {
    winston.debug(
      `handleMessage|No channels configured for the user ${newMessageWithoutContent.fiscalCode} and no default address provided`
    );
...
export async function index(
  ...
  winston.debug(
    `CreatedMessageQueueHandler|A new message was created|${newMessageWithContent.id}|${newMessageWithContent.fiscalCode}`
  );
    ...
    .then(errorOrOutputBindings =>
      errorOrOutputBindings.fold(
        ...
        async outputBindings => {
          ...
          winston.debug(
            `CreatedMessageQueueHandler|succeeded|message=${newMessageWithContent.id}`
          );
