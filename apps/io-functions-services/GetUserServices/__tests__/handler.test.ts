...
describe("GetUserServicesHandler", () => {
  ...
  it("should respond with an internal error if getUser does not respond", async () => {
    const apiClientMock = {
      getUserSubscriptions: vi.fn(() => Promise.reject(new Error("Timeout")))
    };
  ...
  it("should respond with Not found if no user was found", async () => {
    const apiClientMock = {
      getUserSubscriptions: vi.fn(() => Promise.resolve(right({ status: 404 })))
    };
  ...
  it("should respond with an internal error if getUser returns Bad request", async () => {
    const apiClientMock = {
      getUserSubscriptions: vi.fn(() => Promise.resolve(right({ status: 400 })))
    };
  ...
  it("should respond with forbidden if getUser returns Forbidden", async () => {
    const apiClientMock = {
      getUserSubscriptions: vi.fn(() => Promise.resolve(right({ status: 403 })))
    };
