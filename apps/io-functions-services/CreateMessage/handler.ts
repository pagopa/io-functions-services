  ...
  return wrapRequestHandler(
    middlewaresWrap(
      checkSourceIpForHandler(
        ...
        (
          _context: Context,
          _auth: IAzureApiAuthorization,
          clientIp: ClientIp,
          userAttributes: IAzureUserAttributes,
          // eslint-disable-next-line @typescript-eslint/no-unused-vars
          _messagePayload: ApiNewMessageWithDefaults,
          // eslint-disable-next-line @typescript-eslint/no-unused-vars
          _maybeFiscalCode: Option<FiscalCode>,
          // eslint-disable-next-line @typescript-eslint/no-unused-vars
          _req?: express.Request
        ) => ipTuple(clientIp, userAttributes)
      )
    )
  );
}
    ...
    if (hasRemoteContentMessageConfigurationId) {
      ...
      const getRCConfigurationResponseOrError =
        await messagesServicesApiClient.getRCConfiguration({
          configurationId: messagePayload.content.third_party_data
