...
const headA = (a: readonly string[]) => a[0];
const joinS = (a: readonly string[]) => {
...
export const amountArb = fc
  ...
  .map(_ =>
    pipe(
      WithinRangeInteger(1, 9999999999).decode(_),
      E.getOrElseW(() => undefined)
    )
  )
  .filter(_ => _ !== undefined);
...
export const noticeNumberArb = fc
  ...
  .map(_ => _.join("") as PatternString<"^[0123][0-9]{17}$"> & string);
  ...
  .map(
    ([
      ...
      ({
        ...
        maxAllowedPaymentAmount,
        organizationFiscalCode:
          organizationFiscalCode as unknown as PatternString<"^[0-9]{11}$"> &
            string,
